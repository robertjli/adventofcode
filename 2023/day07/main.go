...
var day = util.DayPath(2023, 7)

func main() {
	fmt.Println("Sample Input")
	fmt.Println("------------")
	fmt.Print("Part 1:\t")
	util.Assert(solve("sample.txt", false), 6440)
	fmt.Print("\n")
	fmt.Print("Part 2:\t")
	util.Assert(solve("sample.txt", true), 5905)
	fmt.Print("\n\n")
...
func solve(file string, withJokers bool) int {
	hands := make([]hand, 0, 1000)
	util.ProcessLines(day+file, func(line string) {
		hands = append(hands, parseHand(line, withJokers))
	})
...
func parseHand(line string, withJokers bool) hand {
	var h hand
	...
	for i, c := range cards {
		if unicode.IsDigit(c) {
			h.cards[i] = util.ParseInt(string(c))
		} else if c == 'T' {
			h.cards[i] = 10
		} else if c == 'J' {
			if withJokers {
	...
	h.typ = getType(h.cards)
	h.bid = util.ParseInt(bid)

	return h
}

func getType(cards []int) handtype {
	vals := make([]int, 13)
	var jokers int
	for _, c := range cards {
		if c == 1 {
...
// compareHands compares hands a and b:
// - returns a negative value if a is ranked higher than b
// - returns a positive value if a is ranked lower than b
// - returns 0 if a and b are tied
