...
var day = util.DayPath(2023, 6)

func main() {
	fmt.Println("Sample Input")
	fmt.Println("------------")
	fmt.Print("Part 1:\t")
	util.Assert(solve1("sample.txt"), 288)
	fmt.Print("\n")
	fmt.Print("Part 2:\t")
	util.Assert(solve2("sample.txt"), 71503)
	fmt.Print("\n\n")
...
func solve2(file string) int {
	return waysToWin(parseLongRace(file))
}
...
func parseRaces(file string) []race {
	...
	_, recordsStr, _ := strings.Cut(lines[1], ":")
	records := strings.Fields(recordsStr)

	races := make([]race, len(times))
	for i := range races {
		races[i] = race{
			time:   util.ParseInt(times[i]),
			record: util.ParseInt(records[i]),
		}
	}
	return races
}

func parseLongRace(file string) race {
	lines := util.ReadFile(day + file)
	_, time, _ := strings.Cut(lines[0], ":")
	_, record, _ := strings.Cut(lines[1], ":")

	return race{
		time:   util.ParseInt(strings.ReplaceAll(time, " ", "")),
		record: util.ParseInt(strings.ReplaceAll(record, " ", "")),
	}
