...
50, 51, ..., 96, 97. This corresponds to a destination range starting at 52 and also containing 48
...
func main() {
	...
	fmt.Println("------------")
	fmt.Print("Part 1:\t")
	util.Assert(solve1("sample.txt"), 35)
	fmt.Print("\n")
	fmt.Print("Part 2:\t")
	util.Assert(solve2("sample.txt"), 46)
	fmt.Print("\n\n")
...
func solve1(file string) int {
	...
	vals := parseSeeds(sc.Text())

	sc.Scan()

	for i := 0; i < numMappings; i++ {
		mappings := parseMappings(sc)
		for iv, val := range vals {
			for _, mapping := range mappings {
				if val >= mapping.src && val < mapping.src+mapping.ln {
					vals[iv] = mapping.dest + val - mapping.src
					break
				}
			}
		}
...
func solve2(file string) int {
	...
	ranges := parseSeedRanges(sc.Text())

	sc.Scan()

	next := make([]seedRange, 0, len(ranges)*2)
	for i := 0; i < numMappings; i++ {
		mappings := parseMappings(sc)
		for _, rng := range ranges {
			for _, mapping := range mappings {
				if start, end, ok := intersection(rng, mapping); ok {
					next = append(next, seedRange{start, end})
				}
			}
		}
...
func parseSeeds(line string) []int {
	...
	valStrs := strings.Fields(seeds)
	vals := make([]int, len(valStrs))
	for i := range vals {
		vals[i] = util.ParseInt(valStrs[i])
	}
	return vals
}

func parseSeedRanges(line string) []seedRange {
	vals := parseSeeds(line)
	ranges := make([]seedRange, len(vals)/2)
	for i := range ranges {
		start := vals[i*2]
		ranges[i] = seedRange{start, start + vals[i*2+1] - 1}
	}
	return ranges
}

func parseMappings(sc *bufio.Scanner) []mapping {
	for !strings.ContainsRune(sc.Text(), ':') {
		sc.Scan()
	}
	...
	for len(line) > 0 {
		...
		mappings = append(mappings, mapping{
			dest: util.ParseInt(tokens[0]),
			src:  util.ParseInt(tokens[1]),
			ln:   util.ParseInt(tokens[2]),
		})
		sc.Scan()
		line = sc.Text()
	}
...
func intersection(s seedRange, m mapping) (start, end int, ok bool) {
	if s.end < m.src || s.start >= m.src+m.ln {
		return 0, 0, false
	}
