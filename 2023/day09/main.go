...
var day = util.DayPath(2023, 9)

func main() {
	fmt.Println("Sample Input")
	fmt.Println("------------")
	fmt.Print("Part 1:\t")
	util.Assert(solve1("sample.txt"), 114)
	fmt.Print("\n")
	fmt.Print("Part 2:\t")
	util.Assert(solve2("sample.txt"), 2)
	fmt.Print("\n\n")
...
func solve1(file string) int {
	...
	util.ProcessLines(day+file, func(line string) {
		sum += extrapolate(util.ParseInts(strings.Fields(line)))
	})
	return sum
}
...
func solve2(file string) int {
	var sum int
	util.ProcessLines(day+file, func(line string) {
		ints := util.ParseInts(strings.Fields(line))
		slices.Reverse(ints)
		sum += extrapolate(ints)
	})
	return sum
}
...
func extrapolate(vals []int) int {
	...
	for !same {
		...
		diff := make([]int, len(curr)-1, len(curr))
		same = true
		for i := range diff {
			diff[i] = curr[i+1] - curr[i]
			if i > 0 {
				same = same && diff[i] == diff[i-1]
			}
		}
